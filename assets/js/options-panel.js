window.addEventListener('load', () => {
  ...
  // 生成分组标题：PHP 输出 .fclite-group 标记（带 data-first 组首选项名），按名定位插入
  // （标记落在 form 外也能工作；增删选项无需维护序号）
  document.querySelectorAll('.fclite-group').forEach(marker => {
    const h2El = document.createElement('h2');
    h2El.innerHTML = marker.getAttribute('data-title');
    h2El.classList.add('option-title');
    const first = marker.getAttribute('data-first');
    const anchor = first && optionForm.querySelector('[name="' + first + '"]');
    const li = anchor ? anchor.closest('.typecho-option') : null;
    if (li && li.parentNode) li.parentNode.insertBefore(h2El, li);
    else optionForm.appendChild(h2El);
    marker.remove();
    titleEl.push(h2El);
  });
  ...
  // 插入分隔线（分组标题已按标记就位，只需加线）
  titleEl.forEach(el => {
    if (el.parentNode) el.parentNode.insertBefore(document.createElement('hr'), el);
  });
  ...
  document.querySelector('#export-btn').addEventListener('click', () => {
    ...
    document.querySelector('#download-file').download = 'yclite-config.json';
  ...
  for (let i = 0;i < titleEl.length;i ++) {
    ...
    itemLink.innerHTML = titleEl[i].innerHTML;
